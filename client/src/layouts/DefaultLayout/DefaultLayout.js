...
const sidebarAddOns = {
  [SIDEBAR_ADDON_PROGRESS]: (props) => (
    <SidebarAddOns.ProgressAddOn {...props} /> ),
  [SIDEBAR_ADDON_MENU]: (props) => ( <SidebarAddOns.MenuAddOn {...props} /> ),
  [SIDEBAR_ADDON_BARS]: (props) => ( <SidebarAddOns.BarsAddOn {...props} /> ),
  [SIDEBAR_ADDON_AVATAR_AND_BARS]: (props) => (
    <SidebarAddOns.AvatarAndBarsAddOn {...props} /> ),
  [SIDEBAR_ADDON_AVATAR_AND_NUMBERS]: (props) => (
    <SidebarAddOns.AvatarAndNumbersAddOn {...props} /> ),
  [SIDEBAR_ADDON_AVATAR_AND_STATS]: (props) => (
    <SidebarAddOns.AvatarAndStatsAddOn {...props} /> ),
}
...
class DefaultLayout extends React.Component {
  ...
  componentDidUpdate (prevProps) {
    ...
    this.bodyElement.style.overflow = (this.props.currentScreenSize ===
      SCREEN_SIZE_XS &&
      (
    ...
    const activeRoute = _.first(
      findActiveNodes(ROUTES, this.props.location.pathname))
    document.title = titleBase + activeRoute.title
  }

  render () {
    const staticFootNavContainer =
      !this.props.sidebarEnabled && this.props.contentView ===
      CONTENT_VIEW_STATIC

    const navbarLogo = getLogoBySkin.navbar(this.props.navbarSkin,
      this.props.skinColor),
      sidebarOverlayLogo = getLogoBySkin.sidebar(this.props.sidebarSkin,
        'overlay', this.props.skinColor),
      sidebarBigLogo = getLogoBySkin.sidebar(this.props.sidebarSkin, 'big',
        this.props.skinColor),
      sidebarSlimLogo = getLogoBySkin.sidebar(this.props.sidebarSkin, 'slim',
        this.props.skinColor)
              ...
              {/* Mobile Actions */}
              {
                this.props.currentScreenSize === SCREEN_SIZE_XS && (
                  <div className='pull-right'>
                    ...
                    <button
                      className='btn btn-outline navbar-toggle'
                      onClick={() => this.props.toggleNavbarExpanded(
                        !this.props.navbarExpanded)}
                    ...
                    <button
                      className='btn btn-outline navbar-toggle'
                      onClick={() => this.props.toggleOverlaySidebarOpen(
                        !this.props.overlaySidebarOpen)}
                  ...
                  if (this.props.sidebarAddon === SIDEBAR_ADDON_DEFAULT) {
                    return (
                      <div className="sidebar-logo">
                        <img className="logo-default" width={53}
                             src={sidebarBigLogo}/>
                        <img className="logo-slim" height={13}
                             src={sidebarSlimLogo}/>
                      </div>
                    )
                  } else {
