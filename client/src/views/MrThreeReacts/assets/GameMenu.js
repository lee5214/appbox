    ...
    return (
      <div className={styles.gameMenuContainer}>
        {this.props.gameStatus === "gameOver" ? (
          <h1>You are destroyed by a bomb</h1>
        ) : null}
        {this.props.topScores[0] ? (
          <ListGroup className={styles.scoreBoard}>
            ...
            {this.props.topScores.map(item => (
              <ListGroupItem key={`${item.displayName}-${item.score}`}>
                {item.score} - {item.displayName}
              </ListGroupItem>
            ))}
          </ListGroup>
        ) : (
        ...
        <Row>
          <Button
            ...
            onClick={() => {
              this.props.changeGameStatus("playing");
              this.props.sendGameMessage("Game Start!");
            }}
          >
            Start
          </Button>
        ...
        <Row>
          <Button
            outline
            color={this.state.fullScreen ? "secondary" : "primary"}
            size={"md"}
        ...
        <Row>
          <Col xs={"12"} className={"text-align-center"}>
            ...
            <Button
              ...
              className={"mx-2"}
              size={"sm"}
              onClick={() => this.props.setCameraZ(1000)}
            >
              1000
            </Button>
            <Button
              outline
              size={"sm"}
              onClick={() => this.props.setCameraZ(300)}
            >
              300
            </Button>
          </Col>
        ...
        <Modal isOpen={this.state.modalIsOpen} toggle={this.modalToggle}>
          ...
          <ModalBody>
            <p>
              This is a 3D pilot game I migrated to my app using react &
              threejs, so far it seems work well.
            </p>
            <p>
              Using your cursor to control the plane, try to stay as long as
              possible, game level will increase based on the time lasted. And
              you are able to generate bombs to destroy other players.
            </p>
          </ModalBody>
        </Modal>
      </div>
