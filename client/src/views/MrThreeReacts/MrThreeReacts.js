...
class MrThree extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      cameraZ: 300, //200,
      cameraRotateY: 0,
      skyRotateSpeed: 0.0002,
      seaRotateSpeed: 0.0002,
      energyCharge: 1,
      topScores: [],
      gameSaved: false,
      gameMessage: "",
  ...
  componentDidMount() {
    ...
    rootDB
      ...
      .on("child_added", snapshot => {
        ...
        if (score) {
          this.setState({
            topScores: [score, ...this.state.topScores]
          });
        }
      });
    /*this.setState ({
  ...
  updateDistance() {
    this.Param.distance += Math.floor(
      this.Param.worldSpeed * this.deltaTime * 0.1
    );
  ...
  updatePlane = () => {
    ...
    let targetY = normalize(
      ...
      -1,
      1,
      this.Param.planeDefaultHeight - this.Param.planeAmpHeight,
      this.Param.planeDefaultHeight + this.Param.planeAmpHeight
    );
    this.airplane.mesh.position.y +=
      (targetY - this.airplane.mesh.position.y) * 0.08 * this.Param.worldSpeed;
    this.airplane.mesh.rotation.z =
      (targetY - this.airplane.mesh.position.y) * 0.01 * this.Param.worldSpeed;
    !this.Param.bulletTime
      ? (this.airplane.mesh.rotation.x =
          (this.airplane.mesh.position.y - targetY) *
          0.02 *
          this.Param.worldSpeed)
      : null;
  ...
  updateEnergy = () => {
    ...
    this.Param.energy = Math.floor(
      Math.min(this.Param.energy, this.Param.maxEnergy)
    );
  ...
  animate = () => {
    ...
    if (this.state.gameStatus === "playing") {
      ...
      if (
        Math.floor(this.Param.distance) %
          this.Param.distanceForEnnemiesSpawn ===
          0 &&
        Math.floor(this.Param.distance) > this.Param.enemyLastSpawn
      ) {
      ...
      if (
        Math.floor(this.Param.distance) % this.Param.distanceForLevelUpdate ===
          0 &&
        Math.floor(this.Param.distance) > this.Param.levelLastUpdate
      ) {
        this.Param.levelLastUpdate = Math.floor(this.Param.distance);
        this.Param.level++;
        this.Param.targetBaseSpeed =
          this.Param.initSpeed +
          this.Param.incrementSpeedByLevel * this.Param.level;
        //this.Param.worldSpeed += 0.1;//= this.Param.initSpeed + this.Param.incrementSpeedByLevel *
        // this.Param.level;
      }
      if (
        Math.floor(this.Param.distance) % this.Param.distanceForBomb === 0 &&
        Math.floor(this.Param.distance) > this.Param.bombLastDrop
      ) {
        this.sendGameMessage(
          "You dropped a bomb onto other players' face !",
          2000
        );
      ...
      this.updateEnergy();
      this.Param.baseSpeed +=
        (this.Param.targetBaseSpeed - this.Param.baseSpeed) *
        this.deltaTime *
        0.02;
    ...
    this.enemiesHolder.rotateEnemies(
      ...
      this.enemiesPool,
      this.Param.worldSpeed,
      this.changeWorldSpeed.bind(this),
      this.reduceEnergy.bind(this)
    );

    this.sky.mesh.rotation.z +=
      this.state.skyRotateSpeed * this.Param.worldSpeed * this.deltaTime;
    this.sea.mesh.rotation.z +=
      this.state.seaRotateSpeed * this.Param.worldSpeed * this.deltaTime;
